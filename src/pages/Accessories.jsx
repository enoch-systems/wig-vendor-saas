...
const Accessories = () => {
  ...
  const displayedProducts = useMemo(() => {
    ...
    if (category && category !== 'all') {
      if (category === 'brushes') list = list.filter((p) => p.category === 'brushes');
      if (category === 'oils') list = list.filter((p) => p.category === 'oils');
      if (category === 'tools') list = list.filter((p) => p.category === 'tools');
    }
      ...
      <MountReveal className="min-h-screen bg-gray-50 py-12 md:pt-30" style={{
        ...
        <div className="max-w-8xl mx-auto px-2 md:px-7">
          <div className="flex flex-col md:flex-row md:items-start justify-between mb-6 gap-4">
            <div>
              <p className="text-black font-semibold uppercase">Accessories</p>
            </div>
